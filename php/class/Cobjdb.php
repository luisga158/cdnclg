 ...
 * -. seltblFrmshwplay !action=play.php¡   -> Para uso con play.php, pruebas realizadas
 * --------------------------------------------------------------------------------
 ...
 *
 * -. wrttblcsv($tbl,$nmfile)   ->  Funcion que escribe la tabla $tbl en un archivo csv, en subdirectorio csv de esta carpeta
 *                                  Esto pensado en procesar desde mysql y tener opciones alternativas, aun en desarrollo.
 * ================================================================================
 *
...
class Cobjdb
{
    ...
    // Arrays para definir lista de acciones $accionArVal = Nombre de la acciones (param php), $accionArTxt = Texto mostrado en la lista
    private $accionArVal = array();
    private $accionArTxt = array();
    /* Variables para activar/desactivar alertas y console_log, inician en true. 
        Son utiles para desarrolladores.
        La Funcion auxiliar activeDev segun la opcion 1 activa todo, opcion 0 oculta todo
        opcion 2 solo consola, opcion 3 solo alertas
    */
    private $shwalerts = true;
    private $showconsolelog = true;
    ...
    */
    public function __construct($tablenm, $db, $host, $user, $pass)
    {
        ...
        if ($tablenm == '') {
            ...
            foreach ($info_campo as $valor) {
                ...
                //$this->nullornot[$i] = $valor->not_null;
                $i++;
            }
            // llena data con los datos de la tabla
    ...
    /* Funcion auxiliar para ver alertas y consola con la opcion 1, opcion 0 oculta todo
        opcion 2 solo consola, opcion 3 solo alertas */
    public function activeDev($numOpt)
    {
        if ($numOpt == 0) {
            $this->shwalerts = false;
    ...
    /*public function wrtDatatoJS() {
        $datosrtrn = '';
        $lng = count($this->data);
        for ($i=0; $i < $lng; $i++){
            $lng2 = count($this->data[$i]);
            $reg = array();
            for ($j=0; $j < $lng2; $j++){
                $reg = $this->data[$i];
                if ($j == 0){$datosrtrn = $datosrtrn + "['".$reg[$j]."',";}
                else if (($j+1) == $lng2){$datosrtrn = $datosrtrn + "'".$reg[$j]."']";}
                else {$datosrtrn = $datosrtrn +"'".$reg[$j]."',";}
            }
            $datosrtrn = $datosrtrn +',';
        }
        return $datosrtrn;
    }*/   // Funcion no terminada para regresar dato JS fallo
    /* Getters and Setters */
    ...
    public function showrCObjeto()
    {
        ...
        if ($this->tablenm == '') {
            echo '<select name="listdblcl" id="listdblcl">';
            $i = 0;
    ...
    //
    public function showregtablacrud()
    {
        if (isset($_POST["listdblcl"])) {
            $tblpost = $_POST['listdblcl'];
            $this->tablenm = $tblpost;
        ...
        if ($this->tablenm == '') {
            ...
            if ($this->numfilas > 0) {
                echo "<table>
                <tr>";
                // aqui van cabecera botones
                echo "<th><i class='fa-solid fa-down-long'></i></td>";
                echo '<th><i class="fa-solid fa-right-to-bracket"></i></td>';
                $longitud = count($this->hddata);
                for ($i = 0; $i < $longitud; $i++) {
                    echo '<th>' . $this->hddata[$i] . '</th>';
                }
                ...
                for ($j = 0; $j < $this->numfilas; $j++) {
                    ...
                    echo '<td><button type="button" onclick="alert(' . "'edit reg numero: " . $idl . "'" . ')"><i class="fa-solid fa-pen"></i></button></td>';
                    echo '<td><button type="button" onclick="alert(' . "'borrar reg numero: " . $idl . "'" . ')"><i class="fa-solid fa-trash"></i></button></td>';
                    // datos
                    for ($i = 0; $i < $longitud; $i++) {
                        echo "<td>" . ($rawd[$i]) . "</td>";
                    }
    ...
    // Devuelve registro en array. id = valor a buscar, colid = encabezado donde buscar.
    public function getcoldata($colid)
    {
        if (in_array($colid, $this->hddata)) {
            $longitud = count($this->hddata);
            $arrtrn = array();
            $rawr = $this->data;
            for ($j = 0; $j < $this->numfilas; $j++) {
                $rawd = $rawr[$j];
                $arrtrn[$j] = utf8_encode($rawd[$colid]);
            }
            return $arrtrn;
        }
    }
    ...
    /** Regresa el nombre de la primera tabla donde encontro el $id, de lo contrario retorna null.
        artbls = tablas donde buscar y arids = nombres de los campos donde buscar.              */
    public function idtblfind($id, $artbls, $arids)
    {
        $dtrtr = '';
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        mysqli_select_db($con, $this->db);
        $numtbls = count($artbls);
        for ($i = 0; $i < $numtbls; $i++) {
            $result = mysqli_query($con, "SELECT * FROM " . $artbls[$i] . " WHERE " . $arids[$i] . " = '$id'");
            if ($row = mysqli_fetch_array($result)) {
                $dtrtr = $artbls[$i];
                $i = $numtbls;
            }
    ...
    /** Regresa un array con el nombre de las tablas donde encontro el $id, de lo contrario retorna null
        artbls = tablas donde buscar y arids = nombres de los campos donde buscar.                          */
    public function idtblsfind($id, $artbls, $arids)
    {
        $dtrtr = array();
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        mysqli_select_db($con, $this->db);
    ...
    // Similar al anterio pero no solo tiene opcion de ver tabla, tabien de grabar en csv y js, 
    // Y ahora slide-regs para lo cual vamos a usar mycount como valor 1
    public function seltblFrmselact($action)
    {
        echo '<div class="container">';
        echo '<h5>Seleccione la tabla y la acción deseada</h5>';
        echo '<form class="form" action="' . $action . '" method="POST">';
        echo '<div class="form-group">';
        echo '<select name="nmtbl">';
        foreach ($this->tblsdb as &$valor) {
            echo '<option value="' . $valor . '" name="tablesel">' . $valor . '</option>';
        }
        echo "</select>";
        ...
        echo '<option value="editregtabla">Editar</option>';
        echo '<option value="shwtbl">Ver Tabla</option>';
        echo '<option value="wrttblcsv">Wrt CSV</option>';
        echo '<option value="wrttbljs">Wrt JS</option>';
        echo '<option value="wrtbljstolib">Wrt JS to View con lib</option>';
        echo '<option value="slideregs">Slide Reg</option>';
        echo '<option value="tablatrib">Ver Propiedades de tabla</option>';
        echo "</select>";
        echo '</div>';
        echo '<input type="submit" class="btn btn-success btn-sm" value=" Ejecutar " />';
        echo '</form>';
        echo '</div>';
    }
    /* Usado con play.php otra version de program    */
    public function seltblFrmshwplay()
    {
        echo '<form class="form" action="play.php" method="POST">';
        echo '<fieldset class="pure-group">';
        echo '<select name="listdblcl">';
        foreach ($this->tblsdb as &$valor) {
            echo '<option value="' . $valor . '" name="tablesel">' . $valor . '</option>';
        ...
        echo "</select><br />";
        echo '</fieldset>';
        echo '<input id="btnobjdbfrm" type="submit" value=" Seleccionar " />';
        echo '</form>';
    }
    ...
    */
    public function dataLst($arlist, $nmlst, $txtin, $pos)
    {
        $lnglst = count($arlist);
        if ($pos == 1) {
            echo '<input list="' . $nmlst . '" name="' . $nmlst . '" type="text" placeholder="' . $txtin . '" autofocus>';
        } else {
    ...
    public function darwFormdataLst($arlist, $nmlst)
    {
        echo '<form class="form" action="php/enviar.php" method="POST">';
        echo '<fieldset class="pure-group">';
        $this->dataLst($arlist, $nmlst, '', 1);
        echo '<input name="Consultar al tercero" type="submit" value=" Consultar " tabindex="2"  />';
        echo '<input type="text" size="auto" name="tablenm" value="lista" hidden />';
        echo '</fieldset><br />';
        echo '</form>';
    }
    ...
    public function darwTpField($tpdt, $dtfill, $atfill, $pos, $nm)
    {
        switch ($tpdt) {
            ...
            case 'texto':
                ...
                break;
            case 'textare':
                echo '<textarea type="text" size="auto" name="' . $nm . '" value="' . utf8_encode($dtfill) . '" tabindex="' . $pos . '"></textarea>';
                break;
            case 'mail':
                echo '<input type="text" size="auto" name="' . $nm . '" value="' . utf8_encode($dtfill) . '" tabindex="' . $pos . '" />';
                break;
            case 'city':
                if (is_numeric($dtfill)) {
                    $dtfill = $this->cityNm[array_search($dtfill, $this->cityCd)];
                }
    ...
    **** 1.- Para dibujar formulario con campos segun tipos             */
    public function drawFormdtTypes($tblfill, $tblfillat, $tbldet)
    {
        ...
        for ($i = 0; $i < $longitud; $i++) {
            if (!(in_array($this->hddata[$i], $this->arcolno))) {  // si el encabezado no esta en arcolno dibuja campo                <---
                ...
                if ($nullsn == 'no') {
                    echo '<h5>' . str_replace("_", " ", $this->hddata[$i]) . ' <span class="error">* campo requerido</span></h5>';
                } else {
                    echo '<h5>' . str_replace("_", " ", $this->hddata[$i]) . '</h5>';
                } // encabezado segun tbl
    ...
    /**** 2.- Para dibujar formulario con campos segun tipos advanced mode */
    public function drawFormdtTypesam($tblfill, $tblfillat, $tbldet, $linklg, $accion)
    {
        $rowdet = array();
        $tipodato = '';
        $auxhd = '';
        echo '<p><span class="error">* campo requerido</span></p>';
        echo '<form class="form" action="' . $linklg . '" method="POST" enctype="multipart/form-data">';
        echo '<fieldset class="pure-group">';
        $longitud = count($this->hddata);
        for ($i = 0; $i < $longitud; $i++) {
            if (!(in_array($this->hddata[$i], $this->arcolno))) {  // si el encabezado no esta en arcolno dibuja campo                <---
                ...
                $tipodato = $rowdet[2];     // tipodato, la columna 2 debe contener el tipo de dato, segun $arcoldttipe
                $nullsn = $rowdet[1];     // si es requerido, la columna 1 'null' debe contener no
                //  Dibuja el label h5 para nuestro caso
                if ($nullsn == 'no') {
                    echo '<h5>' . str_replace("_", " ", $this->hddata[$i]) . ' <span class="error">*</span></h5>';
                } else {
                    echo '<h5>' . str_replace("_", " ", $this->hddata[$i]) . '</h5>';
                } // encabezado segun tbl
                $auxhd = $this->hddata[$i]; // Nombre del encabezado actual
        ...
        echo '</fieldset>';
        echo '<input type="text" size="auto" name="tablenm" value="' . $this->tablenm . '" hidden />';
        echo '<input type="text" size="auto" name="action" value="' . $accion . '" hidden />';
        echo '<br><input type="submit" value=" Enviar " tabindex="' . $i . '"  />';
        echo '</form>';
    }
    // Funcion edit
    public function edit($tblname, $form_data, $field_id, $id)
    {
    ...
    // Evalua la información antes de grabar se basa en los datos entregados en $arrtrn -> datos del form y $tbldet -> detalles para validar
            ...
            if (!(in_array($this->hddata[$i], $this->arcolno))) {
                ...
                if ($arrtrn[$i - $cantarno] == '') {
                    if ($null == 'no') {
                        ...
                        echo '<input type="text" size="auto" name="q" value="0" hidden />';
                        echo '</fieldset><br />';
                        echo '</form>';
                        $rtrn = false;
                    }
    ...
    // funcion para grabar los datos del formulario
    public function procFormAlertMode($arrfrm, $modeshwalerts)
    {
        $this->activeDev($modeshwalerts);
        $sql = "INSERT INTO " . $this->tablenm . " (";
        $longitud = count($this->hddata);
        $longno = count($this->arcolno);
        for ($i = 0; $i < $longitud; $i++) {
    ...
    // Funciones con logros realizados
    public function drawFormseeksld($n, $lnk)
    {
        if (-1 == $n) {
            $n = $this->numfilas - 1;
        }
        ...
        echo '<fieldset class="pure-group">';
        echo '<input type="submit" name="atras" value=" Atras " />';
        echo '<input type="submit" name="next" value=" Siguiente " />';
        echo '</fieldset>';
        echo '<fieldset class="pure-group">';
        for ($i = 0; $i < $this->numcols; $i++) {
            echo '<div class="frmhdar">' . str_replace("_", " ", utf8_encode($this->hddata[$i])) . '</div>';
            $lcldata = utf8_encode(strval($lclarr[$i]));
            echo '<input type="text" size="auto" name="' . str_replace(" ", "_", utf8_encode($this->hddata[$i])) . '" value="' . $lcldata . '" />';
        }
        echo '</fieldset><br>';
        echo '</form>';
        /*echo '<br>'.$this->filas;
        echo '<br>'.$this->cols;
        echo '<br>'.$n;*/
    } // Funcion que dibuja form de reg $n va al final al llegar al inicio, y al llegar al final va al inicio
    public function drawFormseeksldJS($n, $lnk)
    {
        if (-1 == $n) {
            $n = $this->numfilas - 1;
        }
        ...
        echo '<fieldset class="pure-group">';
        echo '<input type="button" onclick="showtblsactionautoback()" id="atras" value=" Atras " />';
        echo '<input type="button" onclick="showtblsactionautonext()" id="next" value=" Siguiente " />';
        echo '</fieldset>';
        echo '<fieldset class="pure-group">';
        for ($i = 0; $i < $this->numcols; $i++) {
            echo '<div class="frmhdar">' . str_replace("_", " ", utf8_encode($this->hddata[$i])) . '</div>';
        ...
        echo '</fieldset><br>';
        echo '</form>';
        echo "<script>document.cookie = 'mycount=" . $n . "'</script>";
        /*echo '<br>'.$this->filas;
        echo '<br>'.$this->cols;
        echo '<br>'.$n;*/
    } // Funcion que dibuja form de reg $n va al final al llegar al inicio, y al llegar al final va al inicio para JS

    public function drawFormseeksldJSii($n, $lnk)
    {
        if (-1 == $n) {
            $n = $this->numfilas - 1;
        } else if ($this->numfilas == $n) {
            $n = 0;
        }
        $lclarr = $this->data[$n];
        echo '<form class="form" action="' . $lnk . '" method="POST">';
        ...
        echo '</fieldset><br>';
        echo '<input  type="text" name="nmtbl" value="' . $this->tablenm . '" hidden/>';
        echo '<input  type="text" name="mycount" value="' . $n . '" hidden/>';
        echo '</form>';
        /*echo '<br>'.$this->filas;
        echo '<br>'.$this->cols;
        echo '<br>'.$n;*/
    ...
    {
        if ((0 >= $n) || ($n < $this->numfilas)) {
            $lclarr = $this->data[$n];
            echo '<form class="form" action="' . $lnk . '&mycount=' . $n . '" method="POST">';
            echo '<fieldset class="pure-group">';
            if ($n == 0) {
                echo '<input type="button" name="atras" value=" Atras " disabled />';
            } else {
                echo '<input type="submit" name="atras" value=" Atras " />';
            }
            if (($n + 1) == $this->numfilas) {
                echo '<input type="submit" name="next" value=" Siguiente " disabled />';
            } else {
                echo '<input type="submit" name="next" value=" Siguiente " />';
            }
            echo '</fieldset>';
    ...
    } // Funcion que dibuja form de reg $n desabilita atras al llegar al inicio y siguiente al llegar al final
    public function wrttblcsv($tbl, $nmfile)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (in_array($tbl, $this->tblsdb)) {
            if ($fp = fopen($nmfile . ".csv", "w")) {
    ...
    }   // Escribe los datos de la tabla en un archivo csv
    public function wrttbljs($tbl, $nmfile)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (in_array($tbl, $this->tblsdb)) {
            if ($fp = fopen($nmfile . ".js", "w")) {
                fwrite($fp, '{' . PHP_EOL);
                fwrite($fp, '"' . $tbl . '":[' . PHP_EOL);
                $cantfilas = $this->numfilas;
                $cantcols = $this->numcols;
                $result = mysqli_query($con, "SELECT * FROM " . $tbl);
                for ($i = 0; $i < $cantfilas; $i++) {
                    ...
                    for ($j = 0; $j < $cantcols; $j++) {
                        if ($i + 1 == $cantfilas + 1) {
                            $texto = $texto . $this->hddata[$k] . '" : "' . $rowaux[$j] . '"}';
                        } else if ($j == ($cantcols - 1)) {
                            $texto = $texto . $this->hddata[$k] . '" : "' . $rowaux[$j] . '"},';
                        } else {
                            $texto = $texto . $this->hddata[$k] . '" : "' . $rowaux[$j] . '","';
                        }
        ...
        if ($fp = fopen($nmfile . ".txt", "w")) {
            fwrite($fp, '<table id="id01">' . PHP_EOL . '<tr>' . PHP_EOL);
            $cantcols = $this->numcols;
            for ($i = 0; $i < $cantcols; $i++) {
                fwrite($fp, '<th>' . $this->hddata[$i] . '</th>' . PHP_EOL);
            }
            fwrite($fp, '</tr>' . PHP_EOL . '<tr w3-repeat="' . $this->tablenm . '">' . PHP_EOL);
            for ($i = 0; $i < $cantcols; $i++) {
                fwrite($fp, '<td>{{' . $this->hddata[$i] . '}}</td>' . PHP_EOL);
            }
            fwrite($fp, '</tr>' . PHP_EOL . '</table>');
        }
    }    // Escribe los datos de la tabla en un archivo js
    public function wrttbljstolib($tbl, $nmfile)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (in_array($tbl, $this->tblsdb)) {
            if ($fp = fopen($nmfile . ".js", "w")) {
                fwrite($fp, 'var dataSet = [' . PHP_EOL);
                $cantfilas = $this->numfilas;
                $cantcols = $this->numcols;
                $result = mysqli_query($con, "SELECT * FROM " . $tbl);
                for ($i = 0; $i < $cantfilas; $i++) {
                    ...
                    $rowaux = $this->data[$i];
                    for ($j = 0; $j < $cantcols; $j++) {
                        if (($this->hddata[$j] != "id") && ($this->hddata[$j] != "timestamp")) {
                            if ($i + 1 == $cantfilas + 1) {
                                $texto = $texto . $rowaux[$j] . "'],";
                            } else if ($j == ($cantcols - 1)) {
                                $texto = $texto . $rowaux[$j] . "'],";
                            } else {
                                $texto = $texto . $rowaux[$j] . "','";
                ...
                fwrite($fp, "new DataTable('#example', {" . PHP_EOL);
                fwrite($fp, "columns: [" . PHP_EOL);
                $cantcols = $this->numcols;
                for ($i = 0; $i < $cantcols; $i++) {
                    if (($this->hddata[$i] != "id") && ($this->hddata[$i] != "timestamp")) {
                        if ($j == ($cantcols - 1)) {
                            fwrite($fp, "{ title: '" . $this->hddata[$i] . "'}" . PHP_EOL);
                        } else {
                            fwrite($fp, "{ title: '" . $this->hddata[$i] . "'}," . PHP_EOL);
                        }
                    }
                }
                fwrite($fp, "]," . PHP_EOL . "data: dataSet" . PHP_EOL . "});");
                fclose($fp);
            }
        }
        /*if ($fp = fopen($nmfile.".txt", "w")){
    ...
    public function creatable($sql, $tblnm)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (!$con) {
            die("Connection failed: " . mysqli_connect_error());
        ...
        if (mysqli_query($con, $sql)) {
            echo "Table " . $tblnm . " created successfully";
        } else {
            echo "Error creating table: " . mysqli_error($con);
        }
        mysqli_close($con);
    }
    // Se cambia un campo en $tablename, con $ido, en la $nmcol por $valo a setCampo
    public function setCampo($tblname, $ido, $nmcol, $valo)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (!$con) {
            die("Connection failed: " . mysqli_connect_error());
        }
        $sql = "UPDATE " . $tblname . " SET " . $nmcol . "=" . $valo . " where id=" . $ido;
        $result = mysqli_query($con, $sql);
        return $result;
    }
    // Se elimina un registro en $tablename, con $ido
    public function suprimeId($tblname, $ido)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (!$con) {
            die("Connection failed: " . mysqli_connect_error());
        }
        $sql = "DELETE FROM " . $tblname . " where id=" . $ido;
        $result = mysqli_query($con, $sql);
        return $result;
    }
    // Actualiza con datos entregados en $ardat, en el registro con el $ido
    public function updateConId($tblname, $arnmcols, $ardat, $ido)
    {
        $con = mysqli_connect($this->host, $this->user, $this->pass, $this->db);
        if (!$con) {
            die("Connection failed: " . mysqli_connect_error());
        }
        $cantdata = count($ardat);
        $sql = "UPDATE " . $tblname . " SET ";
        for ($i = 0; $i < $cantdata; $i++) {
            $sql .= $arnmcols[$i] . " = '" . $ardat[$i] . "'";
            if ($i + 1 < $cantdata) {
                $sql .= ", ";
            }     // mientras no sea el ultimo dato pone coma
        }
        $sql .= " WHERE id=" . $ido;
        $result = mysqli_query($con, $sql);
